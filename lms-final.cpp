...
class FineSystem : public LibraryResource {
private:
    int memberId;
    double fineAmount;
public:
    void setFine(int id, int mId, double amount) {
        resourceId = id;
        memberId = mId;
        fineAmount = amount;
    }
    void displayInfo() const override {
        cout << "[Fine Log] ID: " << resourceId << " | Member: " << memberId 
             << " | Amount: $" << fineAmount << " | Date: " << entryDate << endl;
    }
};

class Procurement : public LibraryResource {
private:
    string vendorName;
    double price;
public:
    void setPurchase(int id, string vName, double p) {
        resourceId = id;
        vendorName = vName;
        price = p;
    }
    void displayInfo() const override {
        cout << "[Purchase Log] ID: " << resourceId << " | Vendor: " << vendorName 
             << " | Price: $" << price << " | Date: " << entryDate << endl;
    }
};

class ActivityLog : public LibraryResource {
private:
    string activityType;
    string status;
public:
    void setLog(int id, string type, string stat) {
        resourceId = id;
        activityType = type;
        status = stat;
    }
    void displayInfo() const override {
        cout << "[System Log] ID: " << resourceId << " | Activity: " << activityType 
             << " | Status: " << status << " | Time: " << entryDate << endl;
    }
};
...
private: // Private members of Library
    FineSystem fines[100];
    Procurement purchases[100];
    ActivityLog systemLogs[500];
    int fineCount = 0;
    int purchaseCount = 0;
    int logCount = 0;
